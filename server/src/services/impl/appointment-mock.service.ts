...
export class AppointmentMockService implements AppointmentService {
    ...
    constructor(@repository(TestResultRepository) public testResultRepository : TestResultRepository,) {}
     ...
     * @param patientId
     */
    public getAppointmentType(answers: object[], patientId: string): Promise<number> {
     ...
     * @param patientId
     */
    public getAppointmentDateAtHealthCenter(healthCenterId: string, patientId: string): Promise<Date> {
        return new Promise(result => {
            let returnValue = new Date();
            returnValue.setDate(returnValue.getDate() + Math.floor(Math.random() * 6) + 1);
            result(returnValue);
        });
    }
